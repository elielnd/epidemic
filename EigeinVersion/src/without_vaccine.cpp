...
std::vector<int> spreadEpidemicWithoutVaccination(Eigen::SparseMatrix<double> &socialGraph, double initialInfectedPercentage, double nu, double alpha, double delta, double bellkis, int time)
{
...
int main(int argc, char *argv[])
{
    ...
    int time = TIME;
    std::string outputFile = WO_VACCINE_FILE;
    ...
    for (int r = 0; r < run; r++)
    {
        std::vector<int> infectedIndividualsWithoutVaccination = spreadEpidemicWithoutVaccination(adjacencyMatrix, initialInfectedPercentage, nu, alpha, delta, bellkis, time);
    ...
    std::cout << "\n**********************************" << std::endl;
    std::cout << "*** Spread epidemic without vaccination ***" << std::endl;
    ...
    std::cout << "--- Configuration: ---\n"
              ...
              << "\t-Delta : " << delta << "\n"
              << "\t-RUN : " << run << "\n"
              << "\t-prob d'intection après guérison : " << bellkis << "\n"
